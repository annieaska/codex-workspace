...
`scripts/game-v2-zj717-migrate.sh` 是独立于 cy507 1.3.0 发布包的 zj717 受控迁移脚本，只处理赵杰 OpenWrt `192.168.7.17`（Nebula 管理地址 `192.168.66.30`）、Xbox `192.168.7.108` 和 VPS 上的 `zj717` 自有对象。基础迁移事务 `zj717-20260915T234215Z-86178` 已完成双端技术安装、提交和提交后复验；版本 `1.5.5` 已完成页面状态延迟修正，设备内及 Nebula HTTP 技术验证通过，用户于 2026-09-16 确认页面目前表现良好。Xbox NAT、DNS 和实际联机仍需人工业务确认。检查、施工、验收和恢复说明见 [zj717 迁移手册](docs/zj717-migration.md)。
